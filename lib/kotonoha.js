 ...
 */
export function outputCandidates(inputFile, outputFile, filter) {
 ...
 */
export function minMaxCount(inputFile, outputFile, hitChars = '', useBaseDictionary = false, filter = undefined) {
  const words = readFileSync(inputFile, fileOption).trim().split('\n');
 ...
 * @param {string} chars Target charactors
 */
export function countCharPosition(inputFile, chars) {
  const words = readFileSync(inputFile, fileOption).trim().split('\n');
